  ...
  return (
    <View className="min-h-screen items-center justify-center bg-[#0a0f1e] px-4 py-12">
      <View className="absolute inset-0 overflow-hidden pointer-events-none">
        <View className="absolute -top-40 -left-40 w-96 h-96 rounded-full bg-indigo-500/10 blur-3xl" />
        <View className="absolute -bottom-40 -right-40 w-96 h-96 rounded-full bg-emerald-500/10 blur-3xl" />
      </View>
      ...
      <View className="relative w-full max-w-md">
        ...
        <View className="p-8 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md shadow-2xl">
          <Text className="text-xl font-bold text-white mb-1" style={{ fontFamily: 'Space Mono, monospace' }}>Welcome back</Text>
          <Text className="text-white/50 text-sm mb-7">Sign in to your workspace</Text>
          ...
          <View className="gap-4">
            ...
            <Input label="Password" type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" error={errors.password} autoComplete="current-password" />
            ...
            <View className="flex-row items-center gap-2.5">
              <Button
                ...
                className="w-4 h-4 rounded p-0"
              >
                {remember ? <Text className="text-white text-xs">✓</Text> : null}
              </Button>
              <Text className="text-sm text-white/50">Remember me</Text>
            </View>
          ...
          <View className="flex-row items-center justify-center gap-1 mt-6">
            <Text className="text-sm text-white/40">Don&apos;t have an account?</Text>
            <Link to="register" className="text-indigo-400 hover:text-indigo-300 transition-colors">Create one</Link>
          </View>
        </View>

        <Text className="text-center text-xs text-white/20 mt-4">
          Any email + password works — this is a demo
        </Text>
      </View>
    </View>
  );
