...
import { isNative } from '@ultra-ui-library';
import { useSafeInsets } from '@ultra-ui-library';
...
import { ActivityIndicator } from '@ultra-ui-library';
...
export function AppLayout({ children }: AppLayoutProps) {
  const insets = useSafeInsets(); // 0s on web, real hardware insets on native
  ...
  const offlineBanner = !online && (
    <View className="flex-row items-center gap-2 px-4 py-2 bg-amber-500/20 border-b border-amber-500/30 flex-shrink-0">
      ...
      <Text className="text-amber-300 text-xs">Offline — changes saved locally</Text>
    </View>
  );
  ...
  if (isNative) {
    return (
      <View className="flex-1 bg-[#0a0f1e]">
        ...
        {/* Top bar — paddingTop pushes content below the notch */}
        <View className="bg-[#080d1a]" style={{ paddingTop: insets.top }}>
          <Navbar user={user} notifications={notifications} unreadCount={unreadCount}
                  onMarkAllRead={markAllRead} onMenuOpen={() => {}} />
          {offlineBanner}
        </View>
        ...
        {/* Bottom nav — paddingBottom pushes content above the home indicator */}
        <View className="bg-[#080d1a] border-t border-white/10" style={{ paddingBottom: insets.bottom }}>
          <View className="flex-row">
            {NAV_ITEMS.map(item => {
              const active = pathname === `/${item.route}` || (item.route !== 'dashboard' && pathname.startsWith(`/${item.route}`));
              const color = active ? '#818cf8' : 'rgba(255,255,255,0.4)';
              return (
                <Link key={item.route} to={item.route} className="flex-1 items-center py-2.5 gap-1">
                  <Svg size={22} fill="none" stroke={color} viewBox="0 0 24 24">
                    <Path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={item.d} />
                  </Svg>
                  <Text className="text-[10px] font-medium" style={{ color }}>{item.label}</Text>
                </Link>
              );
            })}
          </View>
  ...
  return (
    <View className="flex flex-row h-[100dvh] bg-[#0a0f1e] overflow-hidden">
      ...
      <View className="flex flex-col flex-1 overflow-hidden min-w-0">
        <Navbar user={user} notifications={notifications} unreadCount={unreadCount}
                onMarkAllRead={markAllRead} onMenuOpen={() => setDrawerOpen(true)} />
        {offlineBanner}
          ...
          {NAV_ITEMS.map(item => {
            ...
            return (
              <Link key={item.route} to={item.route}
                    className={cn('flex-1 flex flex-col items-center gap-1 py-2.5 text-xs font-medium transition-colors',
